...
- Change 1: You can bold the part time faculty data

  Why: This will give an emphasis on the specific data youre trying to highlight

- Change 2: You can add a subtitle about the part time faculty data

  Why: Labeling the points of emphasis, as well as giving description, helps the viewer understand more comprehensively.

- Change 3: You can also keep the Y axis more in line with the data. 

  Why: If the data leaves a lot of open space, you can widen the range that the data lies to have a larger and easier to view scale.
...
staff_long %>%
  ...
  scale_color_manual(values = c(
    ...
    "Full-Time Tenured Faculty"    = "gray60",
    "Full-Time Tenure-Track Faculty" = "gray60",
    "Full-Time Non-Tenure-Track Faculty" = "gray60",
    "Graduate Student Employees"   = "gray60"
  )) +
  ...
  labs(
    title = "Faculty Type Trends Over Time",
    subtitle = "Part-time faculty have grown steadily since 1975",
    x = "Year",
    y = "Percentage of Faculty",
    color = "Faculty Type"
  )
...
I changed the colors of every faculty except part time, to gray, and bolded the part time data with red. I also added a subtitle to explain the growth of part time faculty's. This helps emphasize to the reader/viewer, the data I want them to interpret.
...
- Problem 1: The data needs to be more focused on the left values, theres too much open space on the graph making the data practically unreadable.

- Problem 2: The circle graph has far too many countries to keep track of.

- Problem 3: Many of the countries colors are not represneted, or they're too small to see

- Problem 4: There axis don't detail you on what is being shown.

**Proposed improvements:** For each problem above, suggest a solution:

- Improvement 1: Eliminate the circle graph.

- Improvement 2: Label the axis better.

- Improvement 3: Focus the graph closer to where the data lies.

- Improvement 4: Represent every country.
...
ggplot(fisheries_top20, aes(x = country, y = amount / 1e6, fill = type)) +
  geom_col() +
  coord_flip() +
  ...
  labs(
    title = "Top 20 Countries by Total Fisheries Production",
    subtitle = "Capture vs. Aquaculture (in millions of tonnes)",
    x = "Country",
...
Stacking the countries with two fills helps the viewer seek what they're finding, and compare the aquaculture and capture stats specifically. The axis are also changed to millions for easier comprehension. The countries are also ranked from most to least production. The horizontal bars also keep the countries readability from overlapping. The colors are also a good contrast for easy viewing.



**Did your improved visualization accomplish your goals?** Are there any improvements you proposed that you weren't able to implement? If so, what are they? I would say the improvements did accomplish the goals. The way exercise 5 is organized is specifically quite easy for myself to interpret and explain if I'd need to.
